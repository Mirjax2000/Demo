...
class ClientModelTests(TestCase):
    def test_client_creation(self):
        ...
        self.assertEqual(customer.phone, "+420777123456")
    ...
    def test_slug_is_generated_correctly(self):
        ...
        self.assertEqual(client.phone, "+420777123456")
        self.assertEqual(client.email, "pavel.dvorak@example.com")
        self.assertEqual(client.first_15(), "Pavel Dvořák")

    def test_field_types(self):
        """Strukturalni test — street/phone/email jsou šifrované (GDPR)."""
        from app_sprava_montazi.encryption import (
            EncryptedCharField,
            EncryptedEmailField,
        )

        self.assertIsInstance(Client._meta.get_field("name"), models.CharField)
        self.assertIsInstance(Client._meta.get_field("street"), EncryptedCharField)
        ...
        self.assertIsInstance(Client._meta.get_field("phone"), EncryptedCharField)
        self.assertIsInstance(Client._meta.get_field("email"), EncryptedEmailField)
    ...
    def test_max_length_constraints(self):
        """Strukturalni test — street/phone jsou šifrované → nemají max_length na DB."""
        self.assertEqual(Client._meta.get_field("name").max_length, 75)
        # street a phone jsou EncryptedCharField (TextField) → max_length je None na DB
        self.assertIsNone(Client._meta.get_field("street").max_length)
        self.assertEqual(Client._meta.get_field("city").max_length, 32)
        self.assertEqual(Client._meta.get_field("zip_code").max_length, 5)
        self.assertIsNone(Client._meta.get_field("phone").max_length)
    ...
    def test_slug_changes_when_data_changes(self):
        ...
        self.assertEqual(updated_client.phone, "+420234234234")
...
class ClientMethodTests(TestCase):
    ...
    def test_format_psc_and_phone_from_db(self):
        ...
        # phone je normalizován do E.164 při save()
        self.assertEqual(self.customer_1.phone, "+420602234234")
...
class OrderModelTestV2(TestCase):
    ...
    def test_order_creation_with_delivery_default(self):
        """
        Testuje vytvoření nové objednávky (Order) s předdefinovanými hodnotami.
        """
        date_timedelta = timezone.now().date() + timedelta(days=3)
        ...
        self.assertEqual(order.order_number, "54321")
        self.assertEqual(order.status, Status.ADVICED)
    ...
    def test_order_creation_with_delivery_not_delivery_termin(self):
        ...
        # Zkontrolujeme v DB
        order.refresh_from_db()
        self.assertEqual(order.order_number, "54322")
        self.assertEqual(order.status, Status.NEW)
        self.assertEqual(order.team_type, TeamType.BY_DELIVERY_CREW)
